...
def parse_file(file, single_file=None):
    ...
    if not dataframe.empty:
        ...
        plt.figtext(0.0, 1.15, ''.join(head), fontsize=8, horizontalalignment='left')

        for i, index in enumerate(dataframe.mean().round(4).index):
            print(index,dataframe.mean().round(4)[index])
            plt.figtext(0.8, 0.95+i*0.02, '{}:{}'.format(index,dataframe.mean().round(4)[index]), fontsize=8,
